  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        ...
        toast.error(error.response?.data?.message || "Failed to load book. Please try again.");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-slate-200 rounded-xl mt-8">
          ...
          <p className="text-slate-500 mb-6 max-w-md">
            The eBook you are looking for does not exist or you do not have access to it.
          </p>
        </div>
      )}
    </DashboardLayout>
