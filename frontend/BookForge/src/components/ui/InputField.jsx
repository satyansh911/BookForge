...
const InputField = ({ icon: Icon, label, name, className = "", ...props }) => {
  // Destructure any custom props that shouldn't go to the input element
  const { hideMenu, ...inputProps } = props;
  return (
    <div className='space-y-4 group'>
      ...
      <div className='relative border-b border-border focus-within:border-primary transition-colors pb-2'>
        <input
          ...
          name={name}
          {...inputProps}
          className={`w-full bg-transparent text-lg font-serif placeholder:text-border outline-none transition-all ${Icon ? "pl-8" : ""} ${className}`}
        />
        {Icon && (
